...
export default {
  init(canvas, app, server) {
    // Read initial phase from URL if present
    const initialPhase = new URL(window.location.href).searchParams.get('phase');
    let activePhase: string | null = initialPhase && (PHASES as readonly string[]).includes(initialPhase) ? initialPhase : null;
    ...
    app.onToggled(({ state }) => {
      panel.toggleAttribute('data-active', state);
    });
  },
